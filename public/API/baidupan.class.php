...
class Baidupan
{
     ...
     * @return string
     */
    public function bdstoken()
    {
        $url = 'http://wappass.baidu.com/passport/realnamewidget-init?tpl=tb&v=' . time() . '913';
        $data = $this->get_curl($url, 0, 0, $this->cookie);
        $arr = json_decode($data, true);
        if ($arr['data']['bdstoken']) {
            return $arr['data']['bdstoken'];
        } else {
            return false;
        }
    }
    ...
    {
        $url = 'http://d.pcs.baidu.com/rest/2.0/pcs/file?method=locatedownload&path=' . urlencode($path) . '&ver=2.0&dtype=0&esl=1&ehps=0&app_id=250528&check_blue=1&' . $this->mstring . '&time=' . time() . '225&cuid=E5043F7C37B7BB71B2A94D932F30B2AE%7C257744010452368';
    ...
    {
        $url = 'http://pan.baidu.com/api/rapidupload?clienttype=6&version=2.0.0.3';
